...
def test(eval_set, epoch=0, write_tboard=False):
    ...
    with torch.no_grad():
        ...
        for iteration, (input, indices) in enumerate(test_data_loader, 1):
            ...
            dbFeat.append(vlad_encoding.detach().cpu().numpy())
            tempDB = dbFeat
            tempDB = np.array(tempDB)
            tempDB = tempDB.squeeze()
            qFeat = vlad_encoding.detach().cpu().numpy()
            del input, image_encoding, vlad_encoding tempDB
            if(tempDB.shape[0] < 200):
				continue
    del test_data_loader
    #dbFeat = np.array(dbFeat)
    #dbFeat = dbFeat.squeeze()
    print(dbFeat.shape)
    # extracted for both db and query, now split in own sets
    qFeat = dbFeat[:610].astype('float32')
    dbFeat = dbFeat[611:].astype('float32')
    ...
    for i in range(predictions.shape[0]):
        x = i
        y = predictions[i]
        if(abs(int(x + y + 610) - 1203) < 10): 
            print(x, y + 610, eval_set.images[x], eval_set.images[int(y+610)])
            cnt += 1
    xx = np.array(xx)
    yy = np.array(yy)
    #plt.scatter(xx, yy)
